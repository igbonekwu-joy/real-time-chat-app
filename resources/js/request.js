...
socket.on('friendRequestNotification', ({ toUserId, fromUser }) => {
    ...
    //append user details
    const list = $(`.request-list-${toUserId}`);
    const requestDetails = `<a
                                class="filterDiscussions all single"
                                id="user-list"
                                data-user-id="${fromUser.id}"
                            >
                                ${fromUser.image
                                ?
                                    `<img class="avatar-md" src="/storage/$${ fromUser.image }" data-toggle="tooltip" data-placement="top" title="Janette" alt="avatar">`
                                :
                                    `<div class="avatar-md">
    ...
    //ensure that the user can be selected
    list.find(`[data-user-id="${fromUser.id}"]`).on('click', function (e) {
        e.preventDefault();
        Livewire.dispatch('select-user', { userId: fromUser.id });
    });
});
