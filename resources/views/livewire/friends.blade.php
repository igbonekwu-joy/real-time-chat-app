                        ...
                        <div id="discussions" class="tab-pane fade active show">
                            <div class="search">
                                <form class="form-inline position-relative">
                                    <input type="search" class="form-control" id="conversations" placeholder="Search for friends..." wire:model.live.debounce.300ms="request">
                                ...
                                <div class="list-group request-list-{{ auth()->user()->id }}" id="chats" role="tablist">
                                    ...
                                    @foreach($friends as $friend)
                                        <a
                                            ...
                                            id="user-list"
                                            wire:click.prevent="selectUser({{ $friend->user->id }})"
                                            wire:key="user-{{ $friend->id }}"
                                        >
                                            @if($friend->user->image)
                                                <img class="avatar-md" src={{ asset('storage') . '/'. $friend->user->image }} data-toggle="tooltip" data-placement="top" title="Janette" alt="avatar">
                                            @else
                                                <div class="avatar-md">
